"""Focused fair eval for experiment 056 (054 + per-head temp + learned sigma + risk-weighted pool).

Evaluates the 056 model with the same 500-seed protocol as
fair_eval_054_focused.py, then writes a comparison report against
055, 054, 053, 052, and cached priors (Baseline, ExpectedInput-H10, ViT-only-v3).
"""
...
MODEL_PATH = "experiments/056_h10_attn_refined_v3_highway/results/model.zip"
SHORT = "AnomAttn-Refined-v3"
NAME = "AnomalyAttention + per-head Risk-Temp + learned sigma + risk-weighted pool on v3 (056)"
...
def main():
    ...
    out_dir = Path("experiments/056_h10_attn_refined_v3_highway/results")
    ...
    # Build comparison report against 055 + 054 + 053 + 052 + cached priors.
    ...
    md = []
    md.append(f"# 056 — Refined Anomaly Attention vs. priors\n")
    ...
    md.append("Compared models: AnomalyAttention+per-head Risk-Temp+learned sigma+risk-weighted pool (056), "
              "AnomalyAttention+Risk-Temp+GRU+Aux (055), "
