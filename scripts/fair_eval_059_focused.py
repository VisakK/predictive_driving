"""Focused fair eval for experiment 059 (potential-based shaping + risk-weighted truncation bonus).

059 = 054 architecture + reward shaping with two new mechanisms:
  - PBS (Ng/Harada/Russell 1999): F(s, s') = ρ(s) − γ·ρ(s'), Φ = −ρ.
    Per-step shaping that's policy-invariant — rewards transitions from
    high to low anomaly without changing the optimal policy.
    pbs_weight = 0.05.
  - Risk-weighted truncation bonus: +1.0 × max_risk_seen at episode end
    if truncated (i.e., survived to T_ep without crashing). Sparse signal
    that explicitly rewards "navigated through danger".
    truncation_bonus_weight = 1.0.

No per-step penalty (anomaly_reward_weight = 0.0) — that's the 057/058 path.

Same 500-seed protocol, bit-comparable to 052/053/054/055/056/057/058.
"""
...
SHORT = "AnomAttn-Risk-GRU-PBS-TruncBonus-v3"
NAME = "AnomalyAttention + Risk-Temp + GRU + PBS (w=0.05) + risk-weighted truncation bonus (w=1.0) on v3 (059)"
...
def main():
    ...
    out_dir = Path("experiments/059_h10_attn_risk_gru_pbs_truncbonus_v3_highway/results")
    ...
    prev_058 = json.load(open(
        "experiments/058_h10_attn_risk_gru_rewardshape_fixed_v3_highway/results/fair_eval_focused.json"
    ))
    ...
    rows = [
        (SHORT, agg),
        ("AnomAttn-Risk-GRU-RewShape-Fixed-v3 (058)",
         prev_058["AnomAttn-Risk-GRU-RewShape-Fixed-v3"]["aggregate"]),
    ...
    md = []
    md.append(f"# 059 — Anomaly Attention + Risk-Temp + GRU + PBS + Truncation Bonus vs. priors\n")
    ...
    md.append("059 = 054 architecture + two anomaly-derived reward signals:\n"
              "- **Potential-based shaping** (Ng/Harada/Russell 1999) with Φ = −ρ. "
              "Per-step shaping `F(s,s') = ρ(s) − γ·ρ(s')`, weight 0.05. "
              "Policy-invariant: rewards de-risking transitions without changing the optimal policy.\n"
              "- **Risk-weighted truncation bonus** of weight 1.0: applied once at episode end "
              "if truncated, magnitude = max_risk_seen during episode. Rewards 'navigated "
              "through danger', not just 'survived'.\n"
              "- `anomaly_reward_weight = 0.0` (no per-step penalty — that's 057/058's path).\n"
              "Tests whether a principled (policy-invariant) shaping plus an evasion-conditional "
              "terminal bonus can improve on 054 where 057/058's penalty-based shaping could not.\n")
