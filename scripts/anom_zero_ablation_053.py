"""Anomaly-zero ablation for experiment 053.

Loads the trained 053 (anomaly-attention + risk-temperature cross-attn)
model and evaluates it over the same 500 seeds as
scripts/fair_eval_053_focused.py — but with the `agent_anomaly` field of
every observation zeroed before the policy forward pass. If the ablated
model's crash rate is close to the unmodified 053 result, the H10
anomaly signal is not actually being used by the policy. We expect a
substantial degradation here (the 053 doc predicts 10+ pp), in contrast
to the 052 result where zeroing the anomaly barely moved the metrics.
"""
...
MODEL_PATH = "experiments/053_h10_attn_risk_v3_highway/results/model.zip"
SHORT = "AnomAttn-Risk-v3-zeroed"
NAME = "AnomalyAttention+RiskTemp 053 with agent_anomaly zeroed"
...
def main():
    ...
    out_dir = Path("experiments/053_h10_attn_risk_v3_highway/results")
    ...
    # Build comparison report against unmodified 053 (cached) and priors.
    focused_053 = json.load(open(out_dir / "fair_eval_focused.json"))
    focused_052 = json.load(open(
    ...
    rows = [
        ("AnomAttn-Risk-v3 (orig)", focused_053["AnomAttn-Risk-v3"]["aggregate"]),
        ("AnomAttn-Risk-v3 (zeroed)", agg_zero),
        ("AnomAttn-v3 (052)", focused_052["AnomAttn-v3"]["aggregate"]),
    ...
    md = []
    md.append("# 053 — Anomaly-Zero Ablation\n")
    ...
    md.append("`AnomAttn-Risk-v3 (zeroed)` = the 053 model evaluated with "
              "`agent_anomaly` set to zero in every observation. The 053 "
              "design forces a gradient pathway from the anomaly channels "
              "through the per-query attention temperature, so we expect "
              "a substantial degradation here — in contrast to the 052 "
              "ablation, where zeroing the anomaly moved crash rate by "
              "only -1.4 pp.\n")
    ...
    md.append("\n## Delta (zeroed - original) for 053\n")
    o = focused_053["AnomAttn-Risk-v3"]["aggregate"]
    z = agg_zero
    md.append(f"- **Crash rate**: {fmt_pct(o['crash_rate'])} -> {fmt_pct(z['crash_rate'])} "
              f"(Delta = {(z['crash_rate'] - o['crash_rate']) * 100:+.1f} pp)")
    md.append(f"- **Crash (adversarial)**: {fmt_pct(o['crash_rate_adversarial'])} -> "
              f"{fmt_pct(z['crash_rate_adversarial'])} "
              f"(Delta = {(z['crash_rate_adversarial'] - o['crash_rate_adversarial']) * 100:+.1f} pp)")
    md.append(f"- **Crash (nominal)**: {fmt_pct(o['crash_rate_nominal'])} -> "
              f"{fmt_pct(z['crash_rate_nominal'])} "
              f"(Delta = {(z['crash_rate_nominal'] - o['crash_rate_nominal']) * 100:+.1f} pp)")
    md.append(f"- **Mean reward**: {o['mean_reward']:.2f} -> {z['mean_reward']:.2f} "
              f"(Delta = {z['mean_reward'] - o['mean_reward']:+.2f})")
    md.append(f"- **Survival**: {fmt_pct(o['truncation_rate'])} -> {fmt_pct(z['truncation_rate'])} "
              f"(Delta = {(z['truncation_rate'] - o['truncation_rate']) * 100:+.1f} pp)")

    md.append("\n## Per-archetype delta (zeroed - original) for 053\n")
    md.append("| Archetype | orig | zeroed | Delta (pp) |")
    md.append("|---|---|---|---|")
    for a in ARCHETYPES:
        ov = o["per_archetype_crash_rate"].get(a, 0.0)
        zv = z["per_archetype_crash_rate"].get(a, 0.0)
        md.append(f"| {a} | {fmt_pct(ov)} | {fmt_pct(zv)} | {(zv - ov) * 100:+.1f} |")
