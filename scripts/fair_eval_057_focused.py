"""Focused fair eval for experiment 057 (054 + anomaly_reward_weight reward shaping).

Evaluates the 057 model with the same 500-seed protocol, then writes
a comparison report against 056, 055, 054, 053, 052, and cached priors.
"""
...
MODEL_PATH = "experiments/057_h10_attn_risk_gru_rewardshape_v3_highway/results/model.zip"
SHORT = "AnomAttn-Risk-GRU-RewShape-v3"
NAME = "AnomalyAttention + Risk-Temp + per-slot GRU + reward shaping (anom_w=0.5) on v3 (057)"
...
def main():
    ...
    out_dir = Path("experiments/057_h10_attn_risk_gru_rewardshape_v3_highway/results")
    ...
    md = []
    md.append(f"# 057 — Anomaly Attention + Risk-Temp + GRU + Reward Shaping vs. priors\n")
    ...
    md.append("057 = 054 architecture + `anomaly_reward_weight=0.5` "
              "(per-step reward penalty proportional to mean H10 risk score). "
              "Tests whether reward shaping with the H10 anomaly improves "
              "on the 054 baseline.\n")
