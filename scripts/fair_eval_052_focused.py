"""Focused fair eval for experiment 052 (anomaly attention).

Evaluates only the 052 model with the same 500-seed protocol as
fair_eval_v3_archetype.py, then writes a comparison report against
cached prior results from experiments/050_h10_v3_highway/results/fair_eval_v3.json
(specifically: Baseline, ExpectedInput-H10, ViT-only-v3).
"""
...
SHORT = "AnomAttn-v3"
NAME = "AnomalyAttention on v3 (052)"
...
def main():
    ...
    # Build comparison report against cached priors.
    prior = json.load(open("experiments/050_h10_v3_highway/results/fair_eval_v3.json"))
    comparison_keys = ["Baseline", "ExpectedInput-H10", "ViT-only-v3"]
    rows = [(SHORT, agg)] + [(k, prior[k]["aggregate"]) for k in comparison_keys]

    md = []
    md.append(f"# 052 — Anomaly Attention vs. priors\n")
    ...
    md.append("Compared models: AnomalyAttention (052), Baseline PPO (027), "
              "ExpectedInput-H10 (048), ViT-only-v3 (051).\n")
