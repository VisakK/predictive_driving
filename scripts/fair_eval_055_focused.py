"""Focused fair eval for experiment 055 (054 + CVAE/Discriminator aux losses).

Evaluates the 055 model with the same 500-seed protocol as
fair_eval_054_focused.py, then writes a comparison report against
054, 053, 052, and cached priors (Baseline, ExpectedInput-H10, ViT-only-v3).
"""
...
SHORT = "AnomAttn-Risk-GRU-Aux-v3"
NAME = "AnomalyAttention + Risk-Temp + per-slot GRU + CVAE/Disc Aux on v3 (055)"
...
def main():
    ...
    # Build comparison report against 054 + 053 + 052 + cached priors.
    prior = json.load(open("experiments/050_h10_v3_highway/results/fair_eval_v3.json"))
    prev_054 = json.load(
        open("experiments/054_h10_attn_risk_gru_v3_highway/results/fair_eval_focused.json")
    )
    prev_053 = json.load(
        open("experiments/053_h10_attn_risk_v3_highway/results/fair_eval_focused.json")
    )
    prev_052 = json.load(
        open("experiments/052_h10_attn_v3_highway/results/fair_eval_focused.json")
    )
    rows = [
        (SHORT, agg),
        ("AnomAttn-Risk-GRU-v3 (054)", prev_054["AnomAttn-Risk-GRU-v3"]["aggregate"]),
        ("AnomAttn-Risk-v3 (053)", prev_053["AnomAttn-Risk-v3"]["aggregate"]),
        ("AnomAttn-v3 (052)", prev_052["AnomAttn-v3"]["aggregate"]),
    ...
    md = []
    md.append(f"# 055 — Anomaly Attention + Risk-Temp + GRU + CVAE/Disc Aux vs. priors\n")
    ...
    md.append("Compared models: AnomalyAttention+Risk-Temp+GRU+Aux (055), "
              "AnomalyAttention+Risk-Temp+GRU (054), "
              "AnomalyAttention+Risk-Temp (053), AnomalyAttention (052), "
              "ExpectedInput-H10 (048), ViT-only-v3 (051), Baseline PPO (027).\n")
