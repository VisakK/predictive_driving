"""Focused fair eval for experiment 054 (053 + per-slot GRU on agent_kin_history).

Evaluates the 054 model with the same 500-seed protocol as
fair_eval_053_focused.py, then writes a comparison report against
053, 052, and cached priors (Baseline, ExpectedInput-H10, ViT-only-v3).
"""
...
SHORT = "AnomAttn-Risk-GRU-v3"
NAME = "AnomalyAttention + Risk-Temp + per-slot GRU on v3 (054)"
...
def main():
    ...
    # Build comparison report against 053 + 052 + cached priors.
    ...
    md = []
    md.append(f"# 054 — Anomaly Attention + Risk-Temp + per-slot GRU vs. priors\n")
    ...
    md.append("Compared models: AnomalyAttention+Risk-Temp+GRU (054), "
